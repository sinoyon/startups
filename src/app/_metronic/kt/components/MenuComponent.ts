...
class MenuComponent {
  ...
  private _isItemDropdownPermanent = (item: HTMLElement) => this._getItemOption(item, 'permanent') === true;

  // Test if item's parent is shown
  private _isItemParentShown = (item: HTMLElement) => getElementParents(item, '.menu-item.show').length > 0;

  // Test of it is item sub element
  private _isItemSubElement = (item: HTMLElement) => item.classList.contains('menu-sub');

  // Test if item has sub
  private _hasItemSub = (item: HTMLElement) => item.classList.contains('menu-item') && item.hasAttribute('data-kt-menu-trigger');
  ...
  private _showDropdown = (item: HTMLElement) => {
    ...
    const inputEl = sub.querySelector('input[type="text"]');
    if (inputEl) {
      inputEl.focus();
    }
  };
