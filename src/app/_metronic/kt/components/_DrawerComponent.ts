...
class DrawerComponent {
  ...
  private _deleteOverlay = () => {
    ...
    // }
    const overlayElement = document.querySelector('body>div.drawer-overlay') as HTMLElement;
    if (overlayElement) {
      document.body.removeChild(overlayElement);
    }
  };

  private _getDirection = () => String(this._getOption('direction')) === 'left' ? 'left' : 'right';
  ...
  public one = (name: string, handler: Function) => EventHandlerUtil.one(this.element, name, handler);

  public off = (name: string) => EventHandlerUtil.off(this.element, name);

  public trigger = (name: string, event: Event) => EventHandlerUtil.trigger(this.element, name, event);
  ...
  public static removeInstance = (element: HTMLElement) => {
    DataUtil.remove(element, 'drawer');
  };
  ...
  public static createInstances(selector: string): void {
    ...
    elements.forEach((element) => {
      ...
      if (drawer) {
        DrawerComponent.removeInstance(item);
      }
