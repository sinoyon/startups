...
export class LayoutService {
  ...
  createOnToolbarSubject = new Subject<any>();
  createVisibleOnToolbar$ = new BehaviorSubject<boolean>(false);
  ...
  toogleToolbar(param = true) {
    const config = this.layoutConfigSubject.value;
    if (!config) {
      return;
    }
    config.toolbar.display = param;
    if (param) {
      KTUtil.ElementStyleUtil.set(document.getElementById('kt_toolbar'),'display', 'flex');
    } else {
      KTUtil.ElementStyleUtil.set(document.getElementById('kt_toolbar'),'display', 'none');
    }
  ...
  toogleAside(param = true) {
    const config = this.layoutConfigSubject.value;
    if (!config) {
      return;
    }
    config.aside.display = param;
    if (param) {
      KTUtil.ElementStyleUtil.set(document.getElementById('kt_aside'),'display', 'flex');
    } else {
      KTUtil.ElementStyleUtil.set(document.getElementById('kt_aside'),'display', 'none');
    }
    const updatedConfig = { ...config};
    this.classes = getEmptyCssClasses();
    this.cssVariables = getEmptyCSSVariables();
